  ...
  try {
    ...
    await connection.execute(
      `CREATE OR REPLACE PROCEDURE fetch_transactions_within_budget (
        ...
        p_cursor OUT SYS_REFCURSOR
          ...
          v_remaining_budget NUMBER := p_budget; -- Budget restant initialisé au montant total
      BEGIN
          OPEN p_cursor FOR
              SELECT id, name, amount, type, account_id, creation_ts
              FROM transactions
              WHERE creation_ts <= (SELECT MIN(creation_ts) FROM transactions WHERE v_total_amount + amount > p_budget) -- Filtrer par la transaction qui fait dépasser le budget
          ...
          LOOP
              ...
              v_remaining_budget := p_budget - v_total_amount; -- Mettre à jour le budget restant
    
              IF v_remaining_budget < 0 THEN
                -- Annuler la dernière transaction récupérée si elle fait dépasser le budget
                CLOSE p_cursor;
                OPEN p_cursor FOR
                    SELECT id, name, amount, type, account_id, creation_ts
                    FROM transactions
                    WHERE id <> v_id; -- Exclure la dernière transaction récupérée
                EXIT;
              END IF;
          END LOOP;
    ...
    await connection.execute(
      ...
      BEGIN
          ...
          IF :NEW.type = 1 THEN -- Inflow
              UPDATE accounts
              SET amount = amount + v_amount_change
              WHERE id = :NEW.account_id;
          ELSIF :NEW.type = 0 THEN -- Outflow
              UPDATE accounts
              SET amount = amount - v_amount_change
              WHERE id = :NEW.account_id;
          END IF;
    ...
    await connection.execute(
      `CREATE OR REPLACE VIEW transactions_view AS
       SELECT id, amount, creation_ts, account_id
       FROM transactions`
    );
...
app.get("/views/:userId", async (req, res) => {
  ...
  const getTransactionsSQL = `SELECT * FROM transactions_view WHERE account_id IN (SELECT id FROM accounts WHERE user_id = :1)`;
