...
`);
  ...
  END;`
    ...
    await connection.execute(`
      ...
      END;
    `);
    await connection.execute(`CREATE VIEW vue_transaction AS SELECT amount,date,account_id,id ONLY FROM transactions;`);
...
app.get("/transactions/anonymous", async (req, res) => {
    const result = await connection.execute(
